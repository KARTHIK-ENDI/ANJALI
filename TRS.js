...
module.exports = bosco = async (bosco, mek) => {
	try {
		...
		const catl = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 400, "message": teks, "footerText": "*ANJALI*", "thumbnail": dfrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             bosco.relayWAMessage(res)
        }
       const catlo = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 70000, "message": teks, "footerText": "Made With Anjali", thumbnail: fs.readFileSync('./ds.jpg'), "surface": 'CATALOG' }}, {quoted:mek})
             bosco.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = bosco.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/LbEtdjqmtxa7jBmCrUdV7V', "groupName": `AKSHAY`, "footerText": "*ANJALI*", "jpegThumbnail": dfrply, "caption": teks}}, {quoted:fvideo})
            bosco.relayWAMessage(grup)
        }
        try {
		pporang = await bosco.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
		pporang = 'https://bit.ly/3sC5w0G'
		      }
        ...
        const reply = (teks) => {
	      bosco.sendMessage(from, teks, text, { thumbnail: denis, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: false, "externalAdReply": { "title": `${' '}ɪᴛs ᴍᴇ ᴀɴᴊᴀʟɪ${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}},})
        }
...
// AUTO
			...
			for (let anju of vien){
				if (budy === anju){
					...
					bosco.sendMessage(from, result, audio, { quoted: mek, mimetype: 'audio/mp4', duration: 99989.0, ptt: true, contextInfo: { forwardingScore: 0, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					bosco.sendMessage(from, result, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 500, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `${fake}`, orderTitle: `MADE BY AKSHAY`, sellerJid: '0@s.whatsapp.net'}}}})
					}
			}
      ...
      const sendWebp = async(to, url) => {
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           bosco.sendMessage(to, media, type, {quoted: mek, "externalAdReply": { "title": `${' '}ɪᴛs ᴍᴇ ᴀɴᴊᴀʟɪ${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      const sendFileFromUrl = async(link, type, options) => {
          ...
          if (!packname) packname = '!TOXIDE'; if (!author) author = 'SER';author = author.replace(/[^a-zA-Z0-9]/g, '');	
...
// Anti link
       ...
       if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
        ...
        if (isCmd && isGroup)
            ...
            case 'owner':
            ini_ownerNumber = [`${setting.owner}@s.whatsapp.net`,`919048329892@s.whatsapp.net`,`919048329892@s.whatsapp.net`]
            let ini_list = []
		    for (let i of ini_ownerNumber) {
			const vname_ = bosco.contacts[i] != undefined ? bosco.contacts[i].vname || bosco.contacts[i].notify : undefined
		     ini_list.push({
			 "displayName": 'Owner Akshay',
			 "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname_ ? `${vname_}` : `${bosco.user.name}`}\nORG: TOXIDE;\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Its me TOXIDE\nEND:VCARD`
			  })
			  }
...
//------------------< Bot Owner >-------------------
       ...
       case 'group':
        rows = [
          ...
          section = [{title: "Sᴜʙsᴄɪʙᴇ Yᴛ ᴛᴏxɪᴅᴇ ɢᴀᴍᴇʀ Fᴏʀ ᴍᴏʀᴇ Uᴘᴅᴀᴛᴇs", rows: rows}]
	...
	case 'tagall':
      ...
      case 'hidetag':
       ...
       case 'anjali':
              if (!q) return
              qq = q.toUpperCase()
              awikwok = `${qq} ${qq} ${qq} ❤️ ❤️ ❤️ WANGY WANGY WANGY WANGY HU HA HU HA HU HA, Anjali Poli Ahn ${qq} Akshay Killadi Ahnu ${qq} AAAAAAAAH ~ Rambutnya.... aaah rambutnya juga pengen aku elus-elus ~~ AAAAAH ${qq} keluar pertama kali di anime juga manis ❤️ ❤️ ❤️ banget AAAAAAAAH ${qq} AAAAA LUCCUUUUUUUUUUUUUUU............ ${qq} AAAAAAAAAAAAAAAAAAAAGH ❤️ ❤️ ❤️apa ? ${qq} itu gak nyata ? Cuma HALU katamu ? nggak, ngak ngak ngak ngak NGAAAAAAAAK GUA GAK PERCAYA ITU DIA NYATA NGAAAAAAAAAAAAAAAAAK PEDULI BANGSAAAAAT !! GUA GAK PEDULI SAMA KENYATAAN POKOKNYA GAK PEDULI. ❤️ ❤️ ❤️ ${qq} gw ... ${qq} di laptop ngeliatin gw, ${qq} .. kamu percaya sama aku ? aaaaaaaaaaah syukur ${q} aku gak mau merelakan ${qq} aaaaaah ❤️ ❤️ ❤️ YEAAAAAAAAAAAH GUA MASIH PUNYA ${qq} SENDIRI PUN NGGAK SAMA AAAAAAAAAAAAAAH`
              reply(awikwok)
              break
        case 'jadibot':
              if (!isOwner) return
...
break
      ...
      case 'photo':
              ...
              bosco.sendMessage(from, buffer, image, {quoted: mek, thumbnail:denis, caption: '©𝘽𝙔 𝘼𝙉𝙅𝘼𝙇𝙄 𝘽𝙊𝙏'})
              fs.unlinkSync(ran)
})
              break
case 'mode':
...
//------------------< Downloader/And Search Media >-------------------
      ...
      case 'more':
       ...
       case 'pinterest':
         case 'pin':
              if (args.length < 1) return reply(`${prefix}anjali`)
...
`
 ...
 menumessage = { contentText: `${teks}`, footerText: `Sᴜʙsᴄʀɪʙᴇ Yᴛ ᴛᴏxɪᴅᴇ ɢᴀᴍᴇʀ`, buttons: menubutton, headerType: 6, locationMessage: bosco2.message.locationMessage}
       ...
       case "anime":
        ...
        case 'waifu':
          ...
          case 'wallml':
              ...
              imageMsg = ( await bosco.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'Dont forget to Subscribe TOXIDEGAMER', imageMessage: imageMsg,
...
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} Akshay`)
       ...
       case 'sgif':
              ...
              if (err) return reply(`${err}`)
              exec(`webpmux -set exif ${addMetadata('TOXIDE')} ${ron} -o ${ron}`, async (error) => {
              if (error) return reply(`${error}`)
...
//------------------< Storage >-------------------
      ...
      case 'addbgm':
					...
					bosco.sendMessage(from, `*Bgm Added*`, MessageType.text, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: 'status@broadcast' } : {})}, message: { orderMessage: { itemCount: 333, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `${nm}`, orderTitle: 'TOXIDE', sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break
	 case 'delvn':
					try {
					 nmm = body.slice(7)
				...
				case 'listbgm':
					...
					teks += `\n*Tᴏᴛᴀʟ : ${vien.length}*\n\n_▢_`
					bosco.sendMessage(from, teks.trim(), extendedText, { caption: 'teks', "contextInfo": { text: 'teks', sendEphemeral: true, "externalAdReply": { "title": `${' '}𝘼𝙉𝙅𝘼𝙇𝙄${''}${''}`, "body": ``, "previewType": 'PHOTO', "thumbnailUrl": `${'https://bit.ly/3sC5w0G'}`, "thumbnail": '', "sourceUrl": `${'https://wa.me/c/919048329892'}`}},quoted: ftext})
					break
				case 'addimage':
...
//------------------< Group >-------------------
       ...
       case 'block':
        ...
        case 'getpp':
               ...
               bosco.sendMessage(from, buffer, image, {caption: "©TOXIDE-SER", quoted: mek })
        ...
        case 'hacked':
              ...
              reply('*Hacked*')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   ...
                   await sleep(1000)
                bosco.groupUpdateSubject(from, `𝙃𝘼𝘾𝙆𝙀𝘿 𝘽𝙔 𝙏𝙊𝙓𝙄𝘿𝙀 𝙎𝙀𝙍`)
                await sleep(1000)
...
//------------------< Maker Menu >-------------------
                ...
                case 'horrorblood':
                case 'thunder':
                    if (args.length == 0) return reply(`Example: ${prefix + command} anjali`)
                    ini_txt = args.join(" ")
...
if (Number(oi2) >= 50) return reply('*Most!*')
	if (!Number(oi2)) return reply('*The number must be a number!*')
     ...
     case 'mute':
                ...
                break
     case 'tts':
					if (args.length < 1) return bosco.sendMessage(from, `Where's the language cod ? contoh : ${prefix}tts ml Akshay uyir`, text, { quoted: mek })
				   const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return bosco.sendMessage(from, `Where's the Texk bro? contoh : ${prefix}tts ml Akshay uyir`, text, { quoted: mek })
